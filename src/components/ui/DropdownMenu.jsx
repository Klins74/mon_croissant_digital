...
const DropdownMenuContent = React.forwardRef(({ className = '', sideOffset = 4, ...props }, ref) => (
  <DropdownMenuPrimitive.Portal>
    <DropdownMenuPrimitive.Content
      ...
      className={`z-50 min-w-[8rem] overflow-hidden rounded-xl border bg-card p-1 text-card-foreground shadow-xl ${className}`}
      asChild
      {...props}
    >
      <MotionDiv
...
const DropdownMenuItem = React.forwardRef(({ className = '', inset, ...props }, ref) => (
  <DropdownMenuPrimitive.Item
    ref={ref}
    className={`relative flex cursor-pointer select-none items-center rounded-lg px-3 py-2 text-sm outline-none transition-colors hover:bg-muted focus:bg-muted data-[disabled]:pointer-events-none data-[disabled]:opacity-50 ${
      inset && 'pl-8'
    } ${className}`}
    {...props}
  />
